...
\begin{frame}[fragile]
  ...
  \begin{definition}
    ...
    \begin{itemize}
      \item $Ref(T) = \bar{x} \mapsto (T \ \bar{s} , \bar{s} ⊑ \bar{x}) ∈ X^* → Set$
      \item{$Ref(f_⋅) = (t, h) \mapsto (f_⋅ \ t , h) ∈ T \stackrel{⋅}{→} S$}
    \end{itemize}
  \end{definition}
  \begin{remark}
...
\begin{frame}[fragile]
  \frametitle{Refining Things to only their Subscipe using the $Ref$ Functor}
  \begin{example}
    \adjustbox{scale=1.05}{    
      \begin{tikzcd}
        & {} \arrow[rrrr, "Ref", dashed, bend left] &   &  &                                                                                & {} &   \\
        Tm  \arrow[rr, "f_⋅"] & {} \arrow[r] & T &  & {\bar{x} \mapsto (Tm \ \bar{s}, \bar{s} ⊑ \bar{x})} \arrow[rr, "{(tm, h) \mapsto (f_⋅ \ tm, h)}"] &    & {\bar{x} \mapsto (T \ \bar{s}, \bar{s} ⊑ \bar{x})}
      \end{tikzcd}
    }
  \end{example}
\end{frame}
...
\begin{frame}[fragile]
  \frametitle{$Ref$ is actually a Monad!}
  \begin{corollary}
    The functor $Ref : \bar{X} → \bar{X}$ gives rise to a monad, i.e. we define two natural transformations
    \begin{itemize}
      \item $unit : T \stackrel{⋅}{→} Ref(T) = t \mapsto (t, id)$ 
      \item $mult : Ref(Ref(T)) \stackrel{⋅}{→} Ref(T) = ((t, h₁) h₂) \mapsto (t, h₁;h₂)$
    \end{itemize}
  \end{corollary}
  \begin{example}
  \end{example}
