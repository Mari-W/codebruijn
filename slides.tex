...
\subtitle{Everybody's Got To Be Somewhere$^{\text{\cite{coDe Bruijn}}}$}
...
\begin{frame}[fragile]
  \frametitle{The Notion of Relevant Pairs}
  \begin{definition}
    We define the set of relevant pairs as:
    \begin{itemize}
      \item $\_×_R\_ : \bar{X} → \bar{X} → \bar{X}  = (T, S, \bar{x}) ↦ ((T ⇑ \bar{x}) × (S ⇑ \bar{x}))$
      \item $\_,_R\_ : T ⇑ \bar{x} → S ⇑ \bar{x} → (T ×_R S) ⇑ \bar{x} = ((t₁, \bar{b}₁), (t₂, \bar{b}₂)) ↦  (((t₁, \bar{b}₁') , (t₂, \bar{b}₂')), \bar{b}')$ \\ 
...
\begin{frame}[fragile]
  \begin{columns}
    \begin{column}{0.5\textwidth}
      \large{\color{bblue} What I've (not) told you:\\This is actually an Agda Paper}
      \vspace{10mm}
      \begin{itemize}
    ...
    \end{column}
    \begin{column}{0.5\textwidth}
      \includegraphics[height=1.15\textheight]{coDe Bruijn-9.pdf}
    \end{column}
  \end{columns}
\end{frame}
